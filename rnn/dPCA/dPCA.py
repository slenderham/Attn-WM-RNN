...
class dPCA(BaseEstimator):
    ...
    [ 0.99244...  0.00755...]
    """
    def __init__(self, labels=None, join=None, n_components=10, regularizer=None, copy=True, n_iter='auto'):
    ...
    def _noise_covariance(self,X,trialX,covariance_type='diagonal'):
        """
        pooled noise covariance across all
        """
        num_trials_per_condition = np.isnan(trialX).astype(int).sum(-1) # number of neurons, time, [var dimensions]
        num_conditions = len(trialX.shape)-2 # 2 dims for neuron and trials
        if covariance_type=='zero':
            Cnoise = np.zeros(X.shape[0])
        elif covariance_type=='identity':
            Cnoise = np.eye(X.shape[0])
        elif covariance_type=='diagonal':
            x_noise = np.nanmean((trialX-X[...,None])**2, axis=-1)
            Cnoise = np.diag(np.sum(x_noise, axis=tuple(range(1,num_conditions+1))))
        elif covariance_type=='full':
            x_noise = (trialX-X[...,None]) # number of neurons, time, [var dimensions], number of samples
            x_noise = x_noise[None,...]*x_noise[:,None,...] # number of neurons ^ 2, time, [var dimensions], number of samples
            Cnoise = np.sum(np.nanmean(x_noise, axis=-1), axis=tuple(range(1,num_conditions+1)))
        return Cnoise

    def _optimize_regularization(self,X,trialX,center=True,lams='auto',max_lam_calls=25):
        ...
        lams = Real(0, 1, "log-uniform", name='lambda')
        # compute crossvalidated score over n_trials repetitions
        objective = lambda lam: np.mean(self.crossval_score(lam,X,trialX,mean=True))

        print("------------------------------------")
        print("starting optimization of lambda")

        res_gp = gp_minimize(objective, lams, n_calls=max_lam_calls, random_state=np.random.randint(1e5), verbose=True)
        lam_star = res_gp.x[0]

        print(f"Optimization complete, cv^*={res_gp.fun}, lambda^*={lam_star}")

        print("------------------------------------")

        # set minimum as new lambda
        self.regularizer = lam_star
    ...
    def crossval_score(self,lam,X,trialX,mean=True):
        ...
        scores = np.zeros((self.n_trials)) if mean \
            else {key : np.zeros((self.n_trials)) for key in list(self.marginalizations.keys())}
    ...
    def _randomized_dpca(self,X,mXs,pinvX=None):
        ...
        for key in list(mXs.keys()):
            ...
            # manually flip sign according to matlab implementation
            # flip_where = np.sign(U).sum(0)<0
            # P[key][:,flip_where] = -P[key][:,flip_where]
            # D[key][:,flip_where] = -D[key][:,flip_where]
    ...
    def _fit(self, X, trialX=None, mXs=None, center=True, SVD=None, optimize=True):
        ...
        if trialX is not None:
            Cnoise = self._noise_covariance(X, trialX)
        else:
            Cnoise = 0
        ...
        if self.regularizer > 0:
            ...
            flat_x = X.reshape((n_features,-1))
            # regX, regmXs, pregX = X, mXs, np.linalg.solve(flat_x@flat_x.T+Cnoise, flat_x).T
            regX, regmXs, pregX = X, mXs, pinv(flat_x)
