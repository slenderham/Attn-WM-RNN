...
class SimpleRNN(nn.Module):
        ...
        if self.plastic:
            ...
            wx = wx * self.oneminusalpha_w + self.dt*R*(
                self.multiply_blocks(torch.einsum('bi, bj->bij', new_output, x), \
                    self.kappa_w[0:2*len(self.input_unit_group)].relu(), self.in_coords))
            if self._sigma_w>0:
            ...
            wh = wh * self.oneminusalpha_w + self.dt*R*(
                self.multiply_blocks(torch.einsum('bi, bj->bij', new_output, output), \
                    self.kappa_w[2*len(self.input_unit_group):2*len(self.input_unit_group)+4].relu(), self.rec_coords))
            if self._sigma_w>0:
    ...
    def print_kappa(self):
        ...
        if self.in_coords is None:
            ...
            print (f'Input->E: ', end='')
            print(self.kappa_w[:len(self.input_unit_group)].relu().tolist())
            print (f'Input->I: ', end='')
            print(self.kappa_w[len(self.input_unit_group):2*len(self.input_unit_group)].relu().tolist())
        ...
        if self.rec_coords is None:
            ...
            print (f'E->E: ', end='')
            print(self.kappa_w[s].relu().item(), end=', ')
            print (f'I->E: ', end='')
            print(self.kappa_w[s+1].relu().item(), end=', ')
            print (f'E->I: ', end='')
            print(self.kappa_w[s+2].relu().item(), end=', ')
            print (f'I->I: ', end='')
            print(self.kappa_w[s+3].relu().item())
